...
from service.service.main_service import ConsultaCep
...
class MainService(Resource):
    ...
    def post(self) -> dict:
        try:
            ...
            main_service = ConsultaCep()
            resp = main_service.executar_rest(dados_request)
            response = objResponse.send_success(data=resp, messages=mensagens.SUCESSO_CONSULTA, status=codeHttp.SUCCESS_200)
